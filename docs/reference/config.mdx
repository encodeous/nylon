...
no_tun: false # if true, run as a userspace-only relay without a TUN (requires use_system_routing: false and no addresses/prefixes for this node in central.yaml)
...
observability_addr: "" # e.g. "0.0.0.0:9090"; enables /metrics, /healthz, /readyz, and /discovery
...
dist:
  ...
  poll_interval: 2m # optional: override central.yaml's polling interval for this node
...
dist:
  ...
  # How often Nylon polls for config updates (optional, default: 10s)
  poll_interval: 30s
  # HTTP sources use cache freshness and conditional requests when supported
  repos:
